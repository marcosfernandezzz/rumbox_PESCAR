    ...
    return (
        <section className="  m-4 py-2 md:m-10 bg-gray-100 shadow-md md:min-h-[450px] rounded-xl">
            ...
            {items.slice(0, 7).map((item, idx) => (
                <Card
                    key={item.id ? item.id : `card-${idx}`}
                    title={item.nombre && item.nombre.length > 15 ? item.nombre.slice(0, 15) + "..." : item.nombre}
                    precio={"$ " + item.precio}
                    image={item.image}
                    descripcion={item.descripcion && item.descripcion.length > 15 ? item.descripcion.slice(0, 35) + "..." : item.descripcion}
                />
            ))}
            </div>
            </div>
