...
export const DescripCompra = () => {
  ...
  const ultimaVentaData = localStorage.getItem("ultimaVenta")
  const copiaUser = localStorage.getItem("usuario")

  // Priorizar datos de la última venta si existe
  const comprador = ultimaVentaData ? JSON.parse(ultimaVentaData) : copiaUser ? JSON.parse(copiaUser) : null
  ...
  const productosRender = comprador.inventario.map((item) => {
    ...
    let imagen = ""
    if (producto) {
      precioUnitario = Number.parseFloat(producto.precio)
      nombre = producto.nombre
      descripcion = producto.descripcion
      imagen = producto.image
    } else if (kit) {
      precioUnitario = Number.parseFloat(kit.precio)
      nombre = kit.nombre
      ...
      >
        ...
        <div className="flex-grow text-center md:text-left">
          ...
          <p className="text-sm text-gray-600 mb-2">
            {descripcion ? descripcion.slice(0, 50) + (descripcion.length > 50 ? "..." : "") : "Sin descripción"}
          </p>
        ...
        <div className="flex flex-col items-end gap-2 min-w-[180px]">
          <p className="text-base text-gray-700">
            Precio unitario:{" "}
            <span className="font-bold text-blue-700">${new Intl.NumberFormat("es-AR").format(precioUnitario)}</span>
          </p>
          <p className="text-base text-gray-700">
            Subtotal:{" "}
            <span className="font-bold text-orange-500">${new Intl.NumberFormat("es-AR").format(subtotal)}</span>
          </p>
        </div>
      </div>
    )
  ...
  const handleVolverInicio = () => {
    // Limpiar los datos de la última venta
    ...
    <section className="container mx-auto p-6 bg-gray-100 min-h-screen">
      {ultimaVentaData && (
        <div className="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded mb-6 text-center">
          ...
          {comprador.fechaCompra && (
            <p className="text-sm mt-2">Fecha: {new Date(comprador.fechaCompra).toLocaleString("es-AR")}</p>
          )}
        </div>
      )}
      ...
      <div className="bg-white shadow-xl rounded-lg p-6 mb-8">
        ...
        <p className="text-lg text-gray-700 mb-2">
          <strong>Nombre:</strong> {comprador.name}
        </p>
        <p className="text-lg text-gray-700">
          <strong>Correo:</strong> {comprador.email}
        </p>
      </div>
      <div>
        ...
        <div className="space-y-6">{productosRender}</div>
        <div className="flex justify-end mt-8">
          <div className="bg-blue-100 rounded-lg px-6 py-4 shadow">
            <p className="text-2xl font-bold text-blue-700">
              Total de la compra:{" "}
              <span className="text-orange-500">${new Intl.NumberFormat("es-AR").format(totalCompra)}</span>
            </p>
          </div>
        </div>
        <div className="flex justify-center mt-8">
          <button
            ...
            className="px-8 py-3 bg-blue-600 text-white rounded-lg font-semibold shadow-md hover:bg-blue-700 transition-colors"
          >
            Continuar Comprando
          </button>
        </div>
